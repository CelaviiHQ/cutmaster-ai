...
import type { ContentType, CutIntent, TimelineMode } from "./types";
...
export function resolveCutIntent(
...
): { intent: CutIntent; reason: string } {
  if (numClips > 1) {
    return {
      intent: "multi_clip",
  ...
  if (d < 45) {
    ...
    return { intent: "peak_highlight", reason: `${d.toFixed(0)}s under 45s → peak highlight` };
  }
  if (d < 120) {
    ...
    return { intent: "peak_highlight", reason: `${d.toFixed(0)}s under 2min → peak highlight` };
  }
  if (d < 600) {
    ...
    return { intent: "narrative", reason: `${d.toFixed(0)}s under 10min → narrative arc` };
  }
  return { intent: "narrative", reason: `${d.toFixed(0)}s long-form → narrative arc` };
}
