...
class TestResolveSafeDir:
    ...
    def test_normal_path_unchanged(self):
        from cutmaster_ai.resolve import _resolve_safe_dir
...
class TestValidatePathWithin:
    ...
    def test_valid_path_within(self):
        from cutmaster_ai.resolve import _validate_path_within
...
class TestSerializer:
    ...
    def test_none(self):
        from cutmaster_ai.resolve import _ser
...
class TestConstants:
    ...
    def test_marker_colors_count(self):
        from cutmaster_ai.constants import MARKER_COLORS

        assert len(MARKER_COLORS) == 16

    def test_clip_colors_count(self):
        from cutmaster_ai.constants import CLIP_COLORS

        assert len(CLIP_COLORS) == 16

    def test_track_types(self):
        from cutmaster_ai.constants import TRACK_TYPES

        assert {"video", "audio", "subtitle"} == TRACK_TYPES

    def test_pages(self):
        from cutmaster_ai.constants import PAGES
    ...
    def test_composite_modes_has_normal(self):
        from cutmaster_ai.constants import COMPOSITE_MODES
    ...
    def test_export_types_mapping(self):
        from cutmaster_ai.constants import EXPORT_TYPES
    ...
    def test_render_presets(self):
        from cutmaster_ai.constants import RENDER_PRESETS
    ...
    def test_studio_only_features(self):
        from cutmaster_ai.constants import STUDIO_ONLY_FEATURES
    ...
    def test_keyframe_modes(self):
        from cutmaster_ai.constants import KEYFRAME_MODES

        assert KEYFRAME_MODES["All"] == 0
        assert KEYFRAME_MODES["Color"] == 1

    def test_version_types(self):
        from cutmaster_ai.constants import VERSION_TYPES

        assert {"local", "remote"} == VERSION_TYPES

    def test_node_cache_modes(self):
        from cutmaster_ai.constants import NODE_CACHE_MODES
