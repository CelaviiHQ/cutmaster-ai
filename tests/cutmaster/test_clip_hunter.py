...
def _cand(start: float, end: float, score: float, quote: str = "x") -> ClipCandidate:
    return ClipCandidate(
        start_s=start, end_s=end, engagement_score=score,
        quote=quote, suggested_caption="cap", reasoning="because",
    )
...
def test_verbatim_timestamps_enforced():
    ...
    plan = ClipHunterPlan(candidates=[
        ClipCandidate(
            start_s=0.95, end_s=60.9, engagement_score=0.9,
            quote="", suggested_caption="", reasoning="",
        ),
    ])
    errors = validate_clip_hunter_plan(plan, _transcript(), 60.0, 1)
...
def test_negative_duration_rejected():
    plan = ClipHunterPlan(candidates=[
        ClipCandidate(
            start_s=50.0, end_s=40.0, engagement_score=0.8,
            quote="", suggested_caption="", reasoning="",
        ),
    ])
    errors = validate_clip_hunter_plan(plan, _transcript(), 60.0, 1)
...
def test_prompt_includes_target_length_and_count():
    ...
    prompt = director._clip_hunter_prompt(
        preset, _transcript()[:5], {"reorder_allowed": True}, 60.0, 3,
    )
...
def test_candidate_to_segments_falls_back_to_reasoning_when_quote_empty():
    cand = ClipCandidate(
        start_s=0.0, end_s=60.0, engagement_score=0.8,
        quote="", suggested_caption="", reasoning="punchline at the end",
    )
    segs = candidate_to_segments(cand)
