...
applied file is recorded in ``_celavii_schema_migrations`` so subsequent
...
log = logging.getLogger("celavii-resolve.migrations")

_FILENAME_RE = re.compile(r"^(\d{4})_[A-Za-z0-9_]+\.sql$")
_PACKAGE = "celavii_resolve.migrations"


def _ensure_bookkeeping(conn: sqlite3.Connection) -> None:
    conn.execute(
        """
        create table if not exists _celavii_schema_migrations (
...
def _applied(conn: sqlite3.Connection) -> set[str]:
    cur = conn.execute("select name from _celavii_schema_migrations")
    ...
    try:
        ...
        for _seq, name, sql in _discover():
            ...
            try:
                ...
                raise
            conn.execute(
                "insert into _celavii_schema_migrations (name) values (?)",
                (name,),
            )
            conn.commit()
