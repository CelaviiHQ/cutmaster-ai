...
def _prompt(preset: PresetBundle, plan: DirectorPlan, transcript: list[dict]) -> str:
...
4. Keep ``name`` short (< 60 chars). Put detail in ``note``.
...
def suggest_markers(
    ...
    return llm.call_structured(
        agent="marker",
        prompt=_prompt(preset, plan, transcript),
