...
All timeouts are overridable via ``CELAVII_*_TIMEOUT_S`` env vars so an
...
log = logging.getLogger("celavii-resolve.cutmaster.timeouts")
...
# can still stretch.
STT_TIMEOUT_S: int = _env_seconds("CELAVII_STT_TIMEOUT_S", 600)
...
DIRECTOR_TIMEOUT_S: int = _env_seconds("CELAVII_DIRECTOR_TIMEOUT_S", 300)

# Two minutes for the Marker agent and speaker reconciler: smaller prompts,
# no retry loop.
MARKER_TIMEOUT_S: int = _env_seconds("CELAVII_MARKER_TIMEOUT_S", 120)
