...
DECSFILM_LUT = f"{_USER_LUT_BASE}/CutMaster/DECSFILM.cube"
...
def celavii_setup_log_grade(
    ...
    if cst_lut_path:
        ...
        if is_manual:
            # Check CutMaster LUT library first — user may have installed the LUT
            library_lut = get_cst_lut_for_camera(camera)
            if library_lut:
                resolved_cst_path = library_lut
                warnings_global.append(
                    f"Using installed LUT from CutMaster library: {os.path.basename(library_lut)}"
                )
