...
Provider selection (in order of precedence):
  ...
  2. ``CUTMASTER_STT_PROVIDER`` env var.
  3. Default: ``"gemini"``.
...
log = logging.getLogger("cutmaster-ai.cutmaster.stt")
...
def _resolve_default_provider() -> str:
    ...
    Order:
      1. ``CUTMASTER_STT_PROVIDER`` env var (explicit opt-in wins).
    ...
    """
    explicit = os.environ.get("CUTMASTER_STT_PROVIDER")
    if explicit:
...
def transcribe_audio(
    ...
    Args:
        ...
        provider: Optional override. Falls back to ``CUTMASTER_STT_PROVIDER``
