...
CUTMASTER_LUT_DIR = _USER_LUT_BASE / "CutMaster"
...
def _get_vendor_dir(vendor: str) -> Path:
    ...
    return CUTMASTER_LUT_DIR / vendor
...
def celavii_lut_library_status() -> str:
    ...
    # Also scan for any extra .cube files in the CutMaster LUT directory
    extra_luts = []
    if CUTMASTER_LUT_DIR.is_dir():
        for cube in CUTMASTER_LUT_DIR.rglob("*.cube"):
            rel = str(cube.relative_to(CUTMASTER_LUT_DIR))
            known = any(
    ...
    return json.dumps(
        {
            ...
            "lut_directory": str(CUTMASTER_LUT_DIR),
            "cameras": entries,
...
def celavii_install_lut_file(
    ...
    """Install a downloaded LUT file into the CutMaster LUT library.
...
def celavii_scan_lut_folder() -> str:
    ...
    """
    results = {
        "cutmaster_lut_dir": str(CUTMASTER_LUT_DIR),
        "cutmaster_luts": [],
        "system_lut_dir": str(_USER_LUT_BASE),
        "system_vendors": {},
    }

    # Scan CutMaster directory
    if CUTMASTER_LUT_DIR.is_dir():
        for cube in sorted(CUTMASTER_LUT_DIR.rglob("*.cube")):
            rel = str(cube.relative_to(CUTMASTER_LUT_DIR))
            size_kb = cube.stat().st_size / 1024
            results["cutmaster_luts"].append(
                {
                    "path": rel,
    ...
    if _USER_LUT_BASE.is_dir():
        for vendor_dir in sorted(_USER_LUT_BASE.iterdir()):
            if vendor_dir.is_dir() and vendor_dir.name != "CutMaster":
    ...
    results["total_cutmaster"] = len(results["cutmaster_luts"])
