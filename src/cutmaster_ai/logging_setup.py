"""Structured logging for cutmaster-ai.

Two modes, selected by the ``CUTMASTER_LOG_FORMAT`` env var:
...
Log record schema (JSON mode):

    {
      ...
      "level":       "INFO" | "WARNING" | ...,
      "logger":      hyphenated cutmaster-ai.* name,
...
def with_run_id(run_id: str) -> Iterator[None]:
    ...
    at the hand-off point — see ``cutmaster_ai.http.routes.cutmaster``
...
def configure_logging(level: int = logging.INFO) -> None:
    """Install the cutmaster-ai log handler on the root logger.
    ...
    """
    fmt_name = os.getenv("CUTMASTER_LOG_FORMAT", "").strip().lower()
