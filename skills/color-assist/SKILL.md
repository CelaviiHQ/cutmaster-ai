...
1. Call `celavii_color_assist` with the user's intent
2. Present the analysis and CDL values
3. Ask the user if they want to apply the correction
4. If yes, call `celavii_color_assist` again with `apply=True`
5. Suggest grabbing a still: `celavii_grab_still`

### Reference Matching
If the user provides a reference image path:
1. Call `celavii_match_to_reference` with the reference path
...
2. Use `celavii_copy_grade_to_all` to propagate
3. Or use `celavii_batch_apply_lut` if applying a LUT
