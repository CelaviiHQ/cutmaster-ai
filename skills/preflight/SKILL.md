...
### 1. Project Status
- Call `cutmaster_get_current_project` — verify a project is open
- Call `cutmaster_get_current_timeline` — verify a timeline exists

### 2. Media Verification
- Call `cutmaster_verify_timeline_media` — check all clips are online
- Report any offline or missing media with clip names

### 3. Timeline Structure
- Call `cutmaster_list_timeline_items` for video track 1 — verify clips exist
- Check for very short clips (< 2 frames) that might be errors
- Check for gaps (if detectable via frame numbers)

### 4. Render Configuration
- Call `cutmaster_get_render_format_and_codec` — report current format
- Call `cutmaster_get_render_settings` — check TargetDir is set and writable
- Verify resolution matches project settings

### 5. Markers Check
- Call `cutmaster_get_timeline_markers` — report any Red markers (issues)
